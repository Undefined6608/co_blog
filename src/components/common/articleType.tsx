...
export const ArticleType: React.FC<SizeInterface> = ({ param }) => {
	...
	const [childSize, setChildSize] = useState<SizeInterface>({ param: { width: "200px", height: "100%", marginTop: "0" } });
	const handlerChild = () => {
		setChildState(!childState);
		if (childState) return setChildSize({ param: { width: "200px", height: "100%", marginTop: "0" } });
