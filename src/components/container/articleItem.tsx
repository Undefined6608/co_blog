...
export const ArticleItem: React.FC<ArticleItemParam> = ({ param, data }) => {
	...
	return (
		<motion.li className={"articleItem"} style={{ width: param.width, height: param.height, marginTop: param.marginTop }} layout initial={{ scale: 0, y: 50 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0, y: -50 }} transition={{ type: "spring" }} key={data.id} onClick={handlerArticle}>
