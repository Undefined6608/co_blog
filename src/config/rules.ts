...
export const LoginRegExp = {
  illegal:
    /(.*=.*--.*)|(.*(\+|-).*)|(.*\w+(%|\$|#|&)\w+.*)|(.*\|\|.*)|(.*\s+(and|or)\s+.*)|(.*\b(select|update|union|and|or|delete|insert|trancate|char|into|substr|ascii|declare|exec|count|master|info|drop|execute)\b.*)/i,
...
export const formRule = {
  ...
  username: [
    { required: true, message: "电话号码/邮箱不能为空！" },
    {
